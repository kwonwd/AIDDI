...
namespace SourGrape.hongyeop
{
    public class CameraController : MonoBehaviour
    {
        ...
        private PlayerStatusController _playerStatusController; // 플레이어 상태 컨트롤러
        #endregion
        void Start()
        {
            _verticalRotationLimit = 80.0f;
            _distance = 5.0f;
            _mouseSensitivityX = 6.0f;
            _mouseSensitivityY = 2.0f;
            _target = GameObject.FindGameObjectWithTag("Player").transform;
            _cameraAdjustY = 1.0f;
            _playerStatusController = GameObject.FindGameObjectWithTag("Player").GetComponent<PlayerStatusController>();
        }
